...
The menu offers `cosmos`, `daaam`, `shell`, `gpu`, `logs`, `auto-launch`, and `auto-bag`. Choose `daaam` for one prepared container shell shared by ROS launch and rosbag commands. It runs the setup through `export HOI_FPS=4.0` and auto-fills `COSMOS_URL` from the allocated node, for example `http://node061:8000/v1`. Leave `BATCH_NAME` manual inside that shell.

Automated batch mode runs `batch_1` through `batch_7` on the same allocation. Open three IDE terminals and run:
...
Defaults: `DAAAM_AUTO_BATCH_START=1`, `DAAAM_AUTO_BATCH_END=7`, and `DAAAM_AUTO_BAG_DELAY_SECONDS=480`. Automated bag playback is unpaused by default; set `DAAAM_AUTO_BAG_START_PAUSED=1` before launching the job only if you want to keep the old paused behavior. Logs and marker files are written under `$PROJECT/.cache/daaam-cosmos-auto/<run_id>/`.
