...
Use one GPU allocation, then choose roles from `bjob` in separate IDE terminals.

```bash
bjob
# Select: + Start a new job -> daaam-cosmos
```

Leave that allocation terminal open. In each IDE terminal, run:

```bash
bjob connect <job_id>
```

The menu offers `cosmos`, `daaam`, `shell`, `gpu`, `logs`, `auto-launch`, `auto-bag`, and `auto`. Choose `daaam` for one prepared container shell shared by ROS launch and rosbag commands. It runs the setup through `export HOI_FPS=4.0` and auto-fills `COSMOS_URL` from the allocated node, for example `http://node061:8000/v1`. Leave `BATCH_NAME` manual inside that shell.

Automated batch mode runs `batch_1` through `batch_7` on the same allocation. The one-terminal path starts Cosmos, ROS launch, and delayed bag playback together:

```bash
bjob connect <job_id> auto
```

The separate debug roles still exist when you want to inspect one worker at a time:

```bash
bjob connect <job_id> cosmos
bjob connect <job_id> auto-launch
bjob connect <job_id> auto-bag
```

Defaults: `DAAAM_AUTO_BATCH_START=1`, `DAAAM_AUTO_BATCH_END=7`, `DAAAM_AUTO_LAUNCH_READY_PATTERN='Waiting for CameraInfo on topic'`, `DAAAM_AUTO_READY_FALLBACK_SECONDS=480`, `DAAAM_AUTO_COSMOS_MODELS_WAIT_SECONDS=1800`, and `DAAAM_AUTO_BAG_DELAY_SECONDS=0`. For each `batch_N`, the bag worker waits until the matching launch log contains the readiness pattern, validates Cosmos at `${COSMOS_URL%/}/models` (`/v1/models`), then plays `/ros2_ws/data/EGG-Dataset/bags/egg_batch_N_full.bag`. If the readiness text is not seen before the fallback timeout, it starts anyway, but Cosmos `/v1/models` must respond before bag playback begins. Automated bag playback is unpaused by default; set `DAAAM_AUTO_BAG_START_PAUSED=1` before launching the job only if you want to keep the old paused behavior. Logs and marker files are written under `$PROJECT/.cache/daaam-cosmos-auto/<run_id>/`.

```bash
# Manual plain shell equivalent
srun --overlap --pty --jobid <job_id> /bin/bash
```
