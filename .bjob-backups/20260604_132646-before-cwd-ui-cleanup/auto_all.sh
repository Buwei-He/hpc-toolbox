...
cleanup() {
    ...
    kill "$cosmos_watch_pid" 2>/dev/null || true
    kill_tree "$bag_pid"
...
printf '  readiness pattern: %s\n' "${DAAAM_AUTO_LAUNCH_READY_PATTERN:-Waiting for CameraInfo on topic}"
printf '  readiness fallback: %s seconds\n' "${DAAAM_AUTO_READY_FALLBACK_SECONDS:-480}"
printf '  Cosmos /v1/models wait: %s seconds\n' "${DAAAM_AUTO_COSMOS_MODELS_WAIT_SECONDS:-1800}"
printf '  post-ready bag delay: %s seconds\n' "${DAAAM_AUTO_BAG_DELAY_SECONDS:-0}"
printf '  state: %s\n' "$STATE_DIR"
printf '  logs:  %s\n\n' "$LOG_DIR"

printf '[%s] Starting Cosmos worker\n' "$(date -Is)" | tee -a "$LOG_DIR/auto_all.log"
COSMOS_LOG="$LOG_DIR/cosmos-reason2-vllm.log" DAAAM_COSMOS_SUPERVISE=1 \
...
(
    ...
    if [[ ! -e "$STATE_DIR/auto.done" ]]; then
        printf '[%s] Cosmos worker exited before automation finished. Marking auto.stop.\n' "$(date -Is)" >> "$LOG_DIR/auto_all.log"
