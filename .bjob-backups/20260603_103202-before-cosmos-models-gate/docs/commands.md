...
Defaults: `DAAAM_AUTO_BATCH_START=1`, `DAAAM_AUTO_BATCH_END=7`, `DAAAM_AUTO_LAUNCH_READY_PATTERN='Waiting for CameraInfo on topic'`, `DAAAM_AUTO_READY_FALLBACK_SECONDS=480`, and `DAAAM_AUTO_BAG_DELAY_SECONDS=0`. For each `batch_N`, the bag worker waits until the matching launch log contains the readiness pattern, then plays `/ros2_ws/data/EGG-Dataset/bags/egg_batch_N_full.bag`. If the readiness text is not seen before the fallback timeout, it starts anyway. Automated bag playback is unpaused by default; set `DAAAM_AUTO_BAG_START_PAUSED=1` before launching the job only if you want to keep the old paused behavior. Logs and marker files are written under `$PROJECT/.cache/daaam-cosmos-auto/<run_id>/`.
