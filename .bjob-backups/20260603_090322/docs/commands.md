...
The menu offers `cosmos`, `daaam`, `shell`, and `gpu`. Choose `daaam` for one prepared container shell shared by ROS launch and rosbag commands. It runs the setup through `export HOI_FPS=4.0` and auto-fills `COSMOS_URL` from the allocated node, for example `http://node061:8000/v1`. Leave `BATCH_NAME` manual inside that shell.
