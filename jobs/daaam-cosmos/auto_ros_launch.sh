...
for batch_num in $(seq "$BATCH_START" "$BATCH_END"); do
    ...
    launch_started_epoch="$(stat -c %Y "$prefix.launch.started" 2>/dev/null || stat -f %m "$prefix.launch.started")"
    bag_started_epoch="$(stat -c %Y "$prefix.bag.started" 2>/dev/null || stat -f %m "$prefix.bag.started" 2>/dev/null || printf 0)"
    if (( bag_started_epoch < launch_started_epoch )); then
        printf 'Stopping automation because ROS launch ended before fresh bag playback started for %s.\n' "$batch_name" | tee -a "$LOG_DIR/auto_launch.log"
        touch "$STATE_DIR/auto.stop"
        exit 1
    fi
