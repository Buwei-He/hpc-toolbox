...
if [[ -n "${DAAAM_TMUX_SESSION:-}" ]]; then
    SESSION="$DAAAM_TMUX_SESSION"
elif [[ "$STEP_ID" == "0" || "$STEP_ID" == "batch" || "$STEP_ID" == "extern" ]]; then
    SESSION="daaam-${JOB_ID}-main"
else
    SESSION="daaam-${JOB_ID}-${STEP_ID}-$$"
fi
...
if ! command -v tmux >/dev/null 2>&1; then
    echo "tmux is not available; opening DAAAM container directly."
    exec bash "$PROFILE_DIR/enter_daaam_container.sh"
fi

if tmux has-session -t "$SESSION" 2>/dev/null; then
    echo "Attaching existing tmux session: $SESSION"
else
    tmux new-session -d -s "$SESSION" -n daaam         "PROJECT='$PROJECT' SLURMD_NODENAME='$NODE' bash '$PROFILE_DIR/enter_daaam_container.sh'"
    tmux set-option -t "$SESSION" history-limit 50000 >/dev/null 2>&1 || true
    echo "Started tmux session: $SESSION"
fi

tmux pipe-pane -o -t "$SESSION:daaam" "cat >> '$LOG_FILE'" 2>/dev/null || true
...
echo "Set DAAAM_TMUX_SESSION=<name> before bjob connect if you intentionally want to reattach one."
echo ""
